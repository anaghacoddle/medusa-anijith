...
export const PriceListPricesAddProductIdsForm = ({
  ...
  const { table } = useDataTable({
    ...
    enableRowSelection: row => {
      return (
        !!row.original.variants?.length && !row.original.variants?.some(v => variantIdMap[v.id])
      );
    },
    getRowId: row => row.id,
    rowSelection: {
    ...
    () => [
      columnHelper.display({
        ...
        cell: ({ row, table }) => {
          ...
          const isPreselected = row.original.variants?.some(v => variantIdMap[v.id]);
          ...
          if (isPreselected) {
            return (
              <Tooltip content="This product is already in the price list">{Component}</Tooltip>
            );
          }

          if (isDisabled) {
            return <Tooltip content="This product has no variants">{Component}</Tooltip>;
          }
