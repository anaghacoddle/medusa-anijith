...
import { useBatchImageVariants, useProductVariants } from "../../../../../hooks/api";
...
const variantColumnHelper = createColumnHelper<NonNullable<AdminProduct["variants"]>[0]>();

export const VariantsTableForm = ({ productId, image }: VariantsTableFormProps) => {
  ...
  const handleSubmit = form.handleSubmit(async data => {
    ...
    const newVariantIds = Object.keys(variantSelection).filter(k => variantSelection[k]);

    const variantsToAdd = newVariantIds.filter(id => !initialVariantIds.includes(id));

    const variantsToRemove = initialVariantIds.filter(id => !newVariantIds.includes(id));
      ...
      <KeyboundForm onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
        ...
        <RouteDrawer.Footer>
          <div className="flex items-center justify-end gap-x-2">
            ...
            <Button size="small" type="submit" isLoading={isPending} disabled={isPending}>
              {t("actions.save")}
            </Button>
          </div>
        </RouteDrawer.Footer>
