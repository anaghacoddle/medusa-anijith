...
const Payment = ({
  ...
  const totalRefunded = payment.refunds.reduce((acc, next) => next.amount + acc, 0);
    ...
    <div className="divide-y divide-dashed">
      ...
      {showCapture && (
        <div className="bg-ui-bg-subtle flex items-center justify-between px-6 py-4">
          ...
          <Button className="shrink-0" size="small" variant="secondary" onClick={handleCapture}>
    ...
    <div className="divide-y divide-dashed">
      <div className="text-ui-fg-subtle grid grid-cols-[1fr_1fr_20px] items-center gap-x-4 px-6 py-4 sm:grid-cols-[1fr_1fr_1fr_20px]">
        <div className="w-full min-w-[60px] overflow-hidden">
          ...
          <Text size="small" leading="compact">
            {format(new Date(creditLine.created_at as string), "dd MMM, yyyy, HH:mm:ss")}
          </Text>
        </div>
        <div className="hidden items-center justify-end sm:flex">
          <Text size="small" leading="compact" className="capitalize">
    ...
    <div className="flex flex-col divide-y divide-dashed">
      {entries.map(({ type, event }) => {
        switch (type) {
          ...
          case "refund":
            return <Refund key={event.id} refund={event} currencyCode={currencyCode} />;
