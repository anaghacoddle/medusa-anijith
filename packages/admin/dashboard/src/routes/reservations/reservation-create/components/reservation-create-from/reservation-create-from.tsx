...
export const ReservationCreateForm = (props: { inventoryItemId?: string }) => {
  ...
  const [inventorySearch, setInventorySearch] = React.useState<string | null>(null);
  ...
  const selectedInventoryItem = inventory_items?.find(it => it.id === inventoryItemId) as
        ...
        <RouteFocusModal.Body className="flex flex-1 flex-col items-center overflow-auto py-16">
          <div className="flex w-full max-w-[720px] flex-col gap-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Form.Field
                ...
                render={({ field: { value, onChange, ...field } }) => {
                  return (
                    <Form.Item>
                      <Form.Label>{t("inventory.reservation.itemToReserve")}*</Form.Label>
                      <Form.Control>
                        <Combobox
                          onSearchValueChange={(value: string) => setInventorySearch(value)}
                          value={value}
                          onChange={v => {
                            onChange(v);
                          }}
                          ...
                          options={(inventory_items ?? []).map(inventoryItem => ({
            ...
            <div className="text-ui-fg-subtle shadow-elevation-card-rest grid grid-rows-4 divide-y rounded-lg">
              ...
              />
              <AttributeGridRow title={t("fields.sku")} value={selectedInventoryItem?.sku ?? "-"} />
              <AttributeGridRow
                  ...
                  return (
                    <Form.Item>
                      ...
                      <Form.Control>
                        <Input
                          type="number"
                          placeholder={t("inventory.reservation.quantityPlaceholder")}
                          value={value || ""}
                          onChange={e => {
                  ...
                  <Form.Item>
                    ...
                    <Form.Control>
                      <Textarea
                        {...field}
                        disabled={!inventoryItemId || !locationId}
                        placeholder={t("inventory.reservation.descriptionPlaceholder")}
                      />
                    </Form.Control>
                    <Form.ErrorMessage />
                  </Form.Item>
