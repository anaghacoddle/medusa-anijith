...
import useSessionKeepAlive from "../utils/useSessionKeepAlive";
...
export class DashboardApp {
      ...
      Object.entries(plugin.displayModule.displays).forEach(([model, customization]) => {
        if (!displays.has(model as CustomFieldModel)) {
          displays.set(
            ...
            new Map<CustomFieldContainerZone, React.ComponentType<{ data: any }>[]>()
          );
        }
  ...
  render() {
    ...
    const DashboardWrapper: React.FC = () => {
      useSessionKeepAlive(6 * 60 * 1000); // Keep session alive every 6 min
